...
This is a boiled down version of [this example notebook](https://github.com/LukasFrankenQ/district_control/blob/master/examples/basic_example.ipynb),
...
actual ML is a work in progress.
...
After this, the `mpc` object stores the overall control chosen, the related cost.
We recommend to pass as `axis`-object as a keyword argument to the `mpc.mpc_step(ax=ax)` to
obtain an illustration of the results (see the notebook of this example).
